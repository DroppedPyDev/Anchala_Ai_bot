...
async def main_broadcast_handler(m, db):
    ...
    if failed == 0:
        await m.reply_text(
            text=f"✅ ```Broadcasting completed Successfully!``` \n\n**Completed in:** `{completed_in}` \n\n**Total users:** `{total_users}` \n\n**Total done:** `{done}` \n\n**Total success:** `{success}` \n\n**Total failed:** `{failed}`",
            quote=True,
        )
    else:
        await m.reply_document(
            document="broadcast-logs.txt",
            caption=f"```✅ Broadcasting completed Successfully!``` \n\n**Completed in:** `{completed_in}`\n\n**Total users:** `{total_users}` \n\n**Total done:** `{done}` \n\n**Total success:** `{success}` \n\n**Total failed:** `{failed}`",
            quote=True,
        )
...
# If you need Hyprtext in Telegram bot message use code below ! © Abhijith-Sudhakaran 🤧
